...
项目根目录（下称 `$ROOT`）：本文件所在目录。状态在 `~/.claude-tmux-bridge/`。
...
- 配置：`~/.claude-tmux-bridge/config.json`（workdir / 超时 / channels.feishu / channels.telegram）。改完 `stop.sh && start.sh`。
- 日志：`~/.claude-tmux-bridge/bridge.log`（结构化）、`daemon.out`（原样输出）。
- 没回复：1) `status.sh` 看 pane 存活；2) bridge.log 找 `skip sender`（不在白名单）；3) `tmux attach -t claude-bridge` 直接看（`Ctrl-b d` 退出，别 exit）。
