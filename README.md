...
Cli 里的 claude 以 `--dangerously-skip-permissions` 运行（否则权限弹窗会卡死无人值守的 Cli）——**安全边界就是白名单**，确保里面只有你自己。Stop hook 通过 `claude --settings` 注入，只对 Cli 的 session 生效，不影响你日常使用 Claude Code。
